function showUser() {
    var show1 = document.getElementById('container-item-right-head1');
    var table1 = document.getElementById('user-list');
...
fetch('http://localhost:3000/user')
    .then(response => response.json())
    .then(data => {
        var html = ''; // Chuỗi HTML tổng hợp cho tất cả các người dùng
        for (let index = 0; index < data.length; index++) {
            const element = data[index];
            const name = element.name;
            const email = element.email;
            ...
            html += '<td>' + email + '</td>'
            html += '<td>' + password + '</td>'
            html += '<td class="fix-option">' + '<button id="update-user-' + userId + '"><i class="fa-solid fa-screwdriver-wrench"></i></button>' + '<button id="delete-user-' + userId + '"><i class="fa-solid fa-trash"></i></button>' + '</td>'
            html += '</tr>';
        }
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            document.getElementById('update-user-' + userId).addEventListener('click', () => {
                // modal update
                ...
                update_data.addEventListener('click', () => {
                    var e = document.getElementById('update-select');
            ...
            document.getElementById('delete-user-' + userId).addEventListener('click', () => {
                ...
                // Lấy các phần tử DOM
                const show_modal_dl_user = document.getElementById('delete-user-' + userId);
...
function showListFilm() {
    var show2 = document.getElementById('container-item-right-head2');
    var table2 = document.getElementById('list-film');
...
fetch('http://localhost:3000/listfilm')
    ...
    .then(data => {
        for (let index = 0; index < data.length; index++) {
            ...
            html += '<td>' + director + '</td>'
            html += '<td>' + content + '</td>'
            html += '<td class="fix-option">' + '<button id="update-listfilm"><i class="fa-solid fa-screwdriver-wrench"></i></button>' + '<button id="delete-film"><i class="fa-solid fa-trash"></i></button>' + '</td>'
            html += '</tr>'
